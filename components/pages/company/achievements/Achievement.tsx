...
import { AboutAward, Stat } from "@/types/types";
...
const Achievement = ({ stats, awards }: AchievementProps) => {
  ...
  }, [selectedImageIndex, achievements.length, isAnimating]);
  ...
  const handleNext = React.useCallback(() => {
    ...
    setTimeout(() => {
      const newIndex =
        selectedImageIndex === achievements.length - 1
          ? 0
          : selectedImageIndex + 1;
      setSelectedImageIndex(newIndex);
      setTimeout(() => setIsAnimating(false), 50);
    ...
    <section className="py-12 lg:py-20 bg-white dark:bg-gray-900 min-h-screen">
      <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mb-16">
          {achievements.map((achievement: AboutAward, index: number) => (
            <div
              key={achievement.id}
              ...
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-700 dark:border-gray-600 bg-gray-800/50 dark:bg-gray-900/50">
                ...
                >
                  <h2 className="text-xl lg:text-2xl font-display font-bold text-white mb-1">
                    {achievements[selectedImageIndex].title}
                  </h2>
                  <p className="text-gray-400 text-sm lg:text-base">
                    {achievements[selectedImageIndex].description}
                  </p>
                </div>
