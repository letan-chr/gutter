...
const ProductDetails = ({ product, relatedProducts }: ProductDetailsProps) => {
  ...
  const galleryImages = useMemo(() => {
    if (product?.images?.length) {
      return product.images.map((img: any) => getImageUrl(img.image_path));
    }

    if (product?.banner_image) {
      return [getImageUrl(product.banner_image)];
    }
              ...
              {relatedProducts.map((relatedProduct: Product) => (
                ...
                >
                  ...
                  <div className="relative h-48 overflow-hidden">
                    {relatedProduct.banner_image ? (
                      <Image
                        src={getImageUrl(relatedProduct.banner_image)}
